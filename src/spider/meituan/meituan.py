    ...
    while True:
        if len(l) > 0:
            ...
            if mark == 0:  # 抓取成功
                sym = 0
                lock.acquire()
                with open(r'./PythonLearn/src/spider/meituan/meituan.csv', 'a', newline='', encoding='gb18030')as f:
                    write = csv.writer(f)
                    write.writerow(info)
                f.close()
                lock.release()
...
if __name__ == '__main__':
    url_list = []
    with open(r'./PythonLearn/src/spider/meituan/meituan_id.csv', 'r', encoding='gb18030')as f:
        read = csv.reader(f)
        for line in read:
            d_list = ['', '']
