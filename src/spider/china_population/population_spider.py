...
# 人口数量excel文件保存路径
POPULATION_EXCEL_PATH = r'./PythonLearn/src/spider/china_population/population.xlsx'


def spider_population():  # 爬取人口数据
    # 请求参数 sj（时间），zb（指标）
    dfwds1 = '[{"wdcode":"sj","valuecode":"LAST70"}, {"wdcode":"zb","valuecode":"A0301"}]'  # 总人口
    dfwds2 = '[{"wdcode": "sj", "valuecode": "LAST70"}, {"wdcode":"zb","valuecode":"A0302"}]'  # 增长率
    dfwds3 = '[{"wdcode": "sj", "valuecode": "LAST70"}, {"wdcode":"zb","valuecode":"A0303"}]'  # 人口结构
...
def get_population_info(population_dict, json_obj):  # 提取人口数量信息
    datanodes = json_obj['returndata']['datanodes']
    for node in datanodes:
        year = node['code'][-4:]  # 获取年份，[-4:]获取后4位数据
        data = node['data']['data']  # 数据数值
...
def save_excel(population_dict):  # 人口数据生成excel文件
    df = pd.DataFrame(population_dict).T[::-1]
    # .T 是行列转换，该属性T是方法的访问器 transpose()的简写
    # [::-1]就是从最后一个元素到第一个元素复制一遍，即倒序
    ...
    df.columns = ['年份', '年末总人口(万人)', '男性人口(万人)', '女性人口(万人)', '城镇人口(万人)', '乡村人口(万人)', '人口出生率(‰)', '人口死亡率(‰)',
                  ...
                  '少儿抚养比(%)', '老年抚养比(%)']  # columns参数用于指定生成的excel中列的顺序
