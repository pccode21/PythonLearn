...
def get_house_info(list):
    for i in list:
        try:
            response = requests.get('https://gz.lianjia.com/ershoufang/'+str(i)+'.html', headers=headers)
            doc = PyQuery(response.text)
            # 基本属性解析
...
def main():
    lists = get_house_list(100)
    print('共计获取房源：', len(list(set(lists))))
    get_house_info(list(set(lists)))
